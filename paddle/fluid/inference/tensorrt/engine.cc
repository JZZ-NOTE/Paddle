  ...
  if (use_dla_) {
    if (!enable_int8 && !enable_fp16) {
      ...
      for (int i = 0; i < network()->getNbLayers(); i++) {
        ...
        if (layer_name.substr(0, layer_name.find(" ")) == "conv2d") {
          if (dynamic_cast<nvinfer1::IConvolutionLayer *>(layer)
                  ->getNbGroups() > 1) {
            LOG(INFO) << layer_name << " fallback from dla to gpu";
            infer_builder_config_->setDeviceType(layer,
